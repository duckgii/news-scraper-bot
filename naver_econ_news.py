...
# 최종 개수 / 후보 개수
TARGET = 10		# 최종 뽑아 저장할 기사 수
CANDIDATES = 20	# 먼저 시도할 후보 기사 수
...
# ── 본문 추출
def extract_article_text(url: str, lang="ko", min_len=200) -> str | None:
	'''본문 추출 실패/짧으면 None'''
	try:
...
if __name__ == "__main__":
	...
	# 5) 먼저 후보 20개 뽑기(다양성 보장)
	candidates = top_n_with_diversity(groups, base, n=CANDIDATES, sim_threshold=93)

	# 6) 후보들부터 본문→요약 시도 (성공 10개 차면 즉시 종료)
	results = []
	tried_urls = set()
	success = 0

	print(f"[INFO] 후보 {len(candidates)}개에서 시작 (목표 {TARGET}개)")
	def try_one(item, idx_note=""):
		url = item.get("rep_url") or item.get("url")
		title = item.get("rep_title") or item.get("title")
		if url in tried_urls:
			return None
		tried_urls.add(url)
		
		print(f"{idx_note} 본문 추출: {title}")
		...
		return {
			...
			"title": title,
			"url": url,
			"source": item.get("rep_source") or item.get("source"),
			"latest_pubDate": item.get("latest_pubDate") or item.get("pubDate"),
			"short_title": summary.get("short_title") or title,
	...
	for i, g in enumerate(candidates, start=1):
		if success >= TARGET:
			break
		res = try_one(g, idx_note=f"[{i}/{len(candidates)}]")
		if res:
			res["rank"] = success + 1
			results.append(res)
			success += 1
			print(f" → OK (성공 {success}/{TARGET})")
		
	if success < TARGET:
		print(f"[INFO] 후보만으로 {success}/{TARGET} → 추가 기사에서 보충 시도")
		candidate_urls = { (x.get("rep_url") or x.get("url")) for x in candidates }
		for it in base:
			if success >= TARGET:
				break
			if it["url"] in candidate_urls:
				continue
			res = try_one(it)
			if res:
				res["rank"] = success + 1
				results.append(res)
				success += 1
				print(f" → 보충 성공 {success}/{TARGET}")
				
	print(f"[DONE] 최종 {len(results)}개 선정")
	...
	if len(results) >= 3:
		...
		invest_struct = {
			...
			"playbook": {},
			"calendar": [],
			"disclaimer": "※ 오늘은 유효 요약이 부족하여 투자 브리핑을 생성하지 못했습니다."
		}
	...
	out = {
		...
		"invest_structured": invest_struct,
		"top": results
	}
